...
export default function AnalyzeYouTube() {
  ...
  return (
    <motion.div className="relative w-full max-w-2xl overflow-hidden rounded-3xl bg-black/20 p-8 shadow-2xl backdrop-blur-lg" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
      <div className="flex items-center justify-center gap-3">
        ...
        <h1 className="bg-gradient-to-r from-purple-300 to-indigo-300 bg-clip-text text-3xl font-bold text-transparent">YouTube Video Analyzer</h1>
      </div>

      <div className="mt-8">
        <label htmlFor="youtube-url" className="mb-2 block text-sm font-medium text-purple-200">Enter Video URL</label>
        <div className="relative">
          <LinkIcon className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-purple-300/60" />
          <input id="youtube-url" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://www.youtube.com/watch?v=..." className="w-full rounded-xl border border-purple-500/30 bg-white/10 p-4 pl-12 text-lg text-white placeholder-purple-300/60 focus:border-purple-400 focus:ring-2 focus:ring-purple-400" />
        </div>
      </div>

      <div className="mt-6 flex justify-center">
        <motion.button onClick={handleAnalyze} disabled={loading || !url.trim()} className="group relative flex w-full max-w-xs items-center justify-center rounded-xl bg-gradient-to-r from-purple-500 to-indigo-500 px-8 py-4 text-lg font-semibold text-white shadow-lg disabled:cursor-not-allowed disabled:opacity-50" whileHover={{ scale: loading ? 1 : 1.03 }} whileTap={{ scale: loading ? 1 : 0.98 }}>
          {loading ? (<><LoadingSpinner /><span className="ml-2">Analyzing...</span></>) : (<><span>Analyze Video</span><ArrowRight className="ml-2 h-5 w-5 transition-transform group-hover:translate-x-1" /></>)}
        </motion.button>
      </div>
